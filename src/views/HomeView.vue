...
const validateReason = (blur) => {
  if (formData.value.reason.length < 10) {
    if (blur) {
      errors.value.reason = 'Reason must at least be 10 characters'
      hasFriend.value = false
    }
  } else if (formData.value.reason.toLocaleLowerCase().trim().split(' ').includes('friend')) {
    errors.value.reason = 'Great to have a friend'
    hasFriend.value = true
  } else {
      ...
      <div class="col-md-8 offset-md-2">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-3">
            ...
            ></textarea>
            <div v-if="errors.reason" :class="hasFriend ? 'text-success' : 'text-danger'">
              {{ errors.reason }}
            </div>
          </div>
          <div class="mb-3">
